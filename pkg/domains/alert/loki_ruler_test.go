...
func TestGetLokiRules(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		path := r.URL.Path
		var tenant string
		if path == "/api/logs/v1/application/loki/api/v1/rules" {
			tenant = "application"
		} else if path == "/api/logs/v1/infrastructure/loki/api/v1/rules" {
			tenant = "infrastructure"
		} else if path == "/api/logs/v1/audit/loki/api/v1/rules" {
			tenant = "audit"
		} else {
